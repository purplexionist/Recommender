...
def pearson(row1, row2, row1Avg, row2Avg):
	top = 0
	bottomRow1 = 0
	bottomRow2 = 0
	for i in range(len(row1)):
		if row1[i] != 99 and row2[i] != 99:
...
def weightedUser(userScores, avgForIndividual, countForIndividual, first, second, allAvg, allCount):
	tot = 0
	kTot = 0
	...
	for i in range(len(userScores)):
		...
		if row[second] != 99:
			theirAvg = allAvg[i]
			sim = pearson(userRow, row, userAvg, theirAvg)
			kTot += abs(sim)
			tot += sim * row[second]
